...
async def _run_cycles(coordinator, plant, n: int) -> None:
    ...
    with (
        patch(
            "custom_components.zero_grid_controller.control_engine.time",
            _FakeTime,
        ),
        patch.object(
...
async def test_import_while_charging_backs_off_charge_first(hass):
    ...
    coordinator.batteries = [_make_battery(max_charge_w=5000.0)]
    coordinator._engine._current_setpoints["Solar"] = 100.0
    coordinator._engine._current_battery_setpoints["Battery"] = -2000.0
...
async def test_discharge_only_when_pv_maxed(hass):
    ...
    coordinator._engine._current_setpoints["Solar"] = 40.0
...
async def test_pv_recovery_reopens_curtailed_arrays(hass):
    ...
    coordinator.batteries = [_make_battery(max_charge_w=5000.0)]
    coordinator._engine._current_setpoints["Solar"] = 40.0
    coordinator._engine._current_battery_setpoints["Battery"] = -1000.0
